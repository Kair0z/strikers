...
ps_input main_vs(vs_input input, uint instance_id : SV_InstanceID, uint start_instance_id : SV_StartInstanceLocation, uint vertex_id : SV_VertexID)
{
    ...
    ps_input output;
    output.world_position = mul(instance.transform, float4(input.position, 1));
    output.position = mul(k_viewprojection, float4(output.world_position.xyz, 1));
...
float shadow_term(float3 world_position)
{
    if (is_texid_valid(k_texid_shadows))
    {
        ...
        const float4 shadowmap_position = mul(k_mat_to_lightspace, float4(world_position.xyz, 1.0f));
        const float2 sample_coordinate = shadowmap_position.xy * float2(width, height);
        const float shadowmap_depth = tex_shadows.Load(int3(sample_coordinate, 0)).x;
        return float(shadowmap_position.z > shadowmap_depth);
    }
    else return 1.0f; // no shadow
}
...
float4 main_ps(ps_input input) : SV_Target0
{
    ...
    const float shadow = shadow_term(input.world_position);
